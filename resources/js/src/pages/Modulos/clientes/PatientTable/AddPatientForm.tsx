...
const AddPatientForm: React.FC<AddPatientFormProps> = ({ onClose, onAddPatient }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const created = await onAddPatient(encryptedPatient);

      if (created) {
        onClose();
      } else {
        setErrors(['Não foi possível cadastrar o paciente. Verifique os dados e tente novamente.']);
      }
    } catch (error) {
    ...
    <div className="add-patient-form">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-row" style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
          ...
          <div className="form-field" style={{ flex: "1 1 140px", maxWidth: "180px" }}>
