...
class EncryptionService
{
    ...
    {
        $this->encryptionKey = (string) config('security.encryption.key', '');
        $this->encryptionEnabled = (bool) config('security.lgpd.enabled', false);

        if (app()->environment('production') && $this->encryptionEnabled && $this->encryptionKey === '') {
            throw new RuntimeException('ENCRYPTION_KEY must be configured when LGPD encryption is enabled in production.');
        }
    }

    /**
     * Criptografa dados sensiveis quando habilitado.
     */
    ...
    /**
     * Descriptografa dados sensiveis quando habilitado.
     * Mantem compatibilidade com registros legados sem criptografia.
     */
    public function decryptSensitiveData(string $encryptedData): string
    {
        ...
        try {
            ...
            // Compatibilidade com valores persistidos sem criptografia.
            return $encryptedData;
        }
    }

    /**
     * Criptografa dados de paciente conforme LGPD.
     */
    ...
    /**
     * Descriptografa dados de paciente.
     */
    ...
    /**
     * Valida se os dados estao criptografados.
     */
